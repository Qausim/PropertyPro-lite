...
export default () => {
  ...
  describe('success', () => {
    it('should get properties of same type irrespective of case', async () => {
      ...
      expect(res.body.data[0].image_url).to.equal(propertyEntries[1].image_url);
      ...
      expect(res.body.data[1].image_url).to.equal(propertyEntries[2].image_url);
    ...
    it('should get properties having the query as a substring', async () => {
      ...
      expect(res.body.data[1].image_url).to.equal(propertyEntries[3].image_url);
      expect(res.body.data[1].owner_name).to.equal(`${agent.first_name} ${agent.last_name}`);
      expect(res.body.data[1].owner_email).to.equal(agent.email);
      expect(res.body.data[1].owner_phone_number).to.equal(agent.phone_number);
    });
